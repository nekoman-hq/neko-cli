...
func renderTableWithWidth(
	...
	if resp.Status == "error" {
		...
		_, err = renderPropertyValues(resolved, w, widthProvider, styler)
		return err
	}
	resolved, err := responseWithResolvedPresentation(resp)
	if err != nil {
	...
	if listData != nil {
		return renderList(listData, w, styler)
	}
...
func renderList(items any, w io.Writer, styler presentationStyler) error {
	...
	// Print header
	printHeader(w, headers, colWidths, styler)

	// Print separator
	printTableSeparator(w, totalWidth(colWidths), styler)

	// Print rows
	for _, row := range rows {
		printRow(w, headers, row, colWidths, styler)
	}

...
The `plugin-release` unit uses `plugin-release/vX.Y.Z` tags and `.github/workflows/release-plugin-release.yml`. Neko CLI owns state, materialized files, release commit, tag, push, and workflow dispatch. The workflow checks out the dispatched tag, validates `release_sha`, validates the materialized version files and unit config, runs tests, and builds the release plugin with GoReleaser in snapshot mode. In Self-Migration M3 the workflow is not yet a publishing workflow: `goreleaser release`, GitHub Release creation, artifact publishing, commits, tags, and pushes are intentionally disabled until GoReleaser can safely publish only the `plugin-release` artifacts.

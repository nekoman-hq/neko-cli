...
`neko release plugin-index` generates the public plugin registry index from V2 plugin units, `.neko/release.state.json`, and plugin manifests. The generated `plugin-index.json` is not committed as source and is not published by this command. Runtime `neko plugin available`, `install`, and `update` use the `plugin-index.json` asset on the `plugin-registry` GitHub Release as the primary source and do not use `/releases/latest` for plugin discovery. Until M2D publishes that asset, an explicit temporary release-prefix fallback supports existing built-in plugins.

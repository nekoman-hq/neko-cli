...
`neko release init` creates these V2 config/state files for one non-plugin unit. It does not create `.release.neko.json`, append units to an existing V2 repository, initialize plugin unit metadata, or generate workflow templates. Existing V1 projects should use `neko release migrate`; GitHub Actions init requires an existing workflow path below `.github/workflows/`.

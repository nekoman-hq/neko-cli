...
Plugin release units can declare registry metadata directly in `.neko/release.config.json` with `kind: "plugin"` and a `plugin` object containing `name`, `manifest`, `assetPrefix`, and `binaryName`. The metadata is validated with the V2 schema and is the release-system source of truth for adding new plugin units. A future plugin-index milestone will publish this metadata for dynamic plugin discovery; `plugin-index.json` is not implemented yet, and plugin install/update still uses the current registry behavior until that milestone lands.

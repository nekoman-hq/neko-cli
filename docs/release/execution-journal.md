...
Structured release output includes the execution journal path, journal identity, selected unit, next version, tag, workflow, release commit SHA, execution state, dispatch journal path when present, dispatch state, dispatch run URL when resolvable, and recovery guidance. Global `--describe` adds declared structured details and response metadata; global `--verbose` separately adds execution/debug logs. This output is the primary operator map after a handoff or failure.

...
The workflow checks out the dispatched tag, validates the tag and release SHA, validates that `.neko/release.state.json`, `.plugin.release.neko.json`, and `plugin/release/manifest.json` all contain the dispatched version, and confirms the configured V2 unit still uses `tagPrefix: plugin-release/v`, `delivery: github-actions`, and this workflow path. It then runs `go test ./...`, checks `.goreleaser.plugin-release.yaml`, runs a plugin-release-only snapshot build, and publishes with that dedicated config.

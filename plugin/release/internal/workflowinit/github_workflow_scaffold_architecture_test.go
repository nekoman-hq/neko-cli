...
func TestWorkflowInitCapabilityDoesNotImportRootReleaseOrChainHandlers(t *testing.T) {
	for _, path := range []string{
		"boundary.go", "github_workflow_scaffold_command.go", "github_workflow_scaffold_model.go",
		"github_workflow_scaffold_plan.go", "github_workflow_scaffold_response.go",
		"github_workflow_scaffold_source.go", "github_workflow_scaffold_target.go",
		"github_workflow_scaffold_writer.go",
	} {
		source := readWorkflowScaffoldArchitectureSource(t, path)
		for _, forbidden := range []string{
			"plugin/release/pkg/release", "net/http", "GITHUB_TOKEN", "TokenResolver",
			"HandleUnits", "HandleDoctor", "HandleReleaseContextValidation",
		} {
			if strings.Contains(source, forbidden) {
...
func readWorkflowScaffoldArchitectureSource(t *testing.T, path string) string {
	t.Helper()
	if path == "github_actions_workflow_spec.go" {
		path = "../releaseworkflow/workflow.go"
	}

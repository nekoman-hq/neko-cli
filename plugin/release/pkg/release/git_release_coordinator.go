...
func (coordinator *GitReleaseCoordinator) Push(ctx *ReleaseExecutionContext, commitSHA string, result *GitReleaseResult) error {
	...
	if _, err := coordinator.gitOutput(ctx.RepositoryRoot, "push", remote, "refs/tags/"+ctx.Tag+":refs/tags/"+ctx.Tag); err != nil {
		return fmt.Errorf("push V2 unit tag %q failed after release commit %s was pushed for unit %q: %w", ctx.Tag, commitSHA, ctx.Unit.ID, err)
	}
	if result != nil {

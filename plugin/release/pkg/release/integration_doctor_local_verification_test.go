...
func TestRepositoryDoctorRepresentsFocusedLocalVerification(t *testing.T) {
	...
	if result.Summary.Verified != 6 {
		t.Fatalf("verified facts = %d, want 6", result.Summary.Verified)
	}
	if len(result.Verifications) != 6 {
		t.Fatalf("verifications = %d, want 6: %#v", len(result.Verifications), result.Verifications)
	}

...
type integrationDoctorRemoteTokenAccess struct {
	resolver  GitHubActionsDispatchTokenResolver
...
func (access *integrationDoctorRemoteTokenAccess) Resolve(
	ctx context.Context,
) (GitHubActionsDispatchToken, bool) {
	if access.attempted {
		return access.token, access.available
	}
	access.attempted = true
	if access.resolver == nil {
		return GitHubActionsDispatchToken{}, false
	}
	token, err := access.resolver.ResolveGitHubActionsDispatchToken(ctx)
...
type integrationDoctorRemoteContext struct {
	...
	publicToken    GitHubActionsDispatchToken
...
func (inspector integrationDoctorGitHubRemoteInspector) inspectRepository(
...
) (integrationDoctorGitHubRepository, integrationDoctorGitHubReadOutcome, GitHubActionsDispatchToken) {
	repository, outcome := inspector.reader.Repository(ctx, identity, GitHubActionsDispatchToken{})
	if outcome.State != integrationDoctorMissing && outcome.State != integrationDoctorUnauthorized {
		return repository, outcome, GitHubActionsDispatchToken{}
	}
	...
	if !available {
		...
		return integrationDoctorGitHubRepository{}, outcome, GitHubActionsDispatchToken{}
	}

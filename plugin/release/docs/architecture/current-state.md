...
| `internal/releasetool/*` | Canonical release-tool identity, configuration, artifact, version, and invocation facts | `Identity`, `V1BehaviorFor`, tool-specific parsers and rewriters | Infrastructure-free facts are shared by Doctor, planning, materialization, and V1 adapters. |
| `internal/releaseworkflow` | Canonical workflow and repository-target facts | `Spec`, `DispatchContract`, `RepositoryTarget` | Contains no HTTP, journal, token, or presentation policy. |
| `internal/githubdispatch` | One bounded workflow-dispatch POST transport and response sanitization | `Client`, `Submit` | Contains no retry, journal, or lifecycle policy. |
| `internal/releasesource` | Tolerant local V1/V2 source inspection | `Snapshot`, `Read` | Read-only and independent of root Release. |
| `internal/legacyrequirements` | Shared V1 token, tool, and file requirements | `Validate` | Preserves the existing V1 validation order and errors. |

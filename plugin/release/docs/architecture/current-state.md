...
| INV-09 | V2 pushes the release commit before the unit tag. A commit-push failure skips tag push; a tag-push failure does not roll back the pushed commit. | `Push`, `PushCommit`, `PushTag`; active runner order | coordinator push-order and failure tests; active runner failure coverage is missing |
...
| INV-14 | Dispatch persists `request-started` before HTTP. A 2xx response is accepted; 400/401/403/404/422/429 are rejected; transport errors, redirects, 5xx, and unexpected outcomes are unknown. | `GitHubActionsDispatcher.Dispatch`, `classifyGitHubActionsDispatchResponse` | dispatcher/client response, timeout, and redirect tests |
| INV-15 | A terminal dispatch journal (`request-started`, `accepted`, `rejected`, or `unknown`) prevents automatic redispatch. Unknown results are never treated as safe retries. | `DispatchJournalStore.Prepare`, `GitHubActionsDispatcher.Dispatch` | terminal-journal and state transition tests |
| INV-16 | An ambiguous pending commit/tag push blocks resume. Resume also refuses to infer completion from `dispatch-journal-prepared` or `commit-pushed`. | `HandleResume`, `resumeJournal` | pre-commit/blocking tests cover only part of this invariant; successful and ambiguous later-state coverage is missing |
...
| INV-22 | V2 code does not perform destructive automatic rollback after commit/tag/push uncertainty. Local snapshots are restored only before the unsafe boundary; later failures preserve evidence. | active runner, `GitReleaseCoordinator`, `ReleaseTransaction.fail` | source assertions and coordinator push failure tests; runner-level failure injection is missing |
| INV-23 | Execution and dispatch journals contain release facts and hashes, not file bytes or tokens. Dispatch errors are capped and redact the exact token. | journal models, `sanitizeDispatchText`, store permissions | journal/dispatcher secret tests; systematic response/log redaction coverage is missing |
| INV-24 | Public response status/error schemas and error codes are command contracts, but they are currently constructed in multiple packages. Unexpected handler errors become fatal top-level `EXECUTION_ERROR`. | `plugin.Response`, `main.main`, handler response helpers | manifest and selected handler tests; no comprehensive golden contract suite |
...
- filesystem/config/state repositories with failure injection;
...
1. Active V2 runner failure matrix: journal write and side-effect failure before/after materialization, state, stage, commit, tag, dispatch-journal creation, commit push, tag push, and dispatch.
2. Resume matrix: each persisted phase and pending action, successful continuations, terminal dispatch states, expected tag already present, config/remote drift, and all ambiguous operations.
3. Stable command contracts: exact error code/message/details, response metadata command, renderer hint, and deterministic item order for every public command.
...
7. Secret non-disclosure across every error, response, log, execution journal, and dispatch journal failure path.
8. Completed release behavior: handoff-ready exclusion, subsequent version planning, and protection from redispatch.

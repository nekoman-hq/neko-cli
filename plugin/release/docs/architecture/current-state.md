...
- Output: `mapIntegrationDoctorResult` alone constructs the plugin response.
  ...
  and ordered diagnostics. Transport-only human properties provide responsive
  summary, facts, diagnostics, and remediation at normal, narrow, and unknown
  widths. `not_ready` requests exit code `1`; the two ready states request exit
  code `0`.

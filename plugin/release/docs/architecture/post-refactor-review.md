...
- **Current behavior:** Invalid, conflicting, or unsupported journals are rejected and preserved. `neko release evidence` reports redacted typed summaries and diagnostics across release execution, dispatch, migration, V1 compensation, and V2 pair-recovery evidence. Its command-owned presentation declaration gives Core semantic column order without moving Evidence policy into Core; actual-width tables, vertical fallback, and `wide` leave existing JSON data unchanged. Read-only `--identity` detail requires an unambiguous 8-64 character lowercase hexadecimal prefix after family/unit filters and exposes the complete redacted typed record. `neko release evidence-archive` can only archive completed release-execution, completed V1 compensation, or completed V2 pair-recovery evidence after family, exact full identity, digest, and explicit confirmation are rechecked.
